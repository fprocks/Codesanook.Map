...
Content Item, Content type, Content Part and Content Item

### Content Item

**Content** item is a single piece of content, often associated with a single URL (address) on the site. Examples of content items are pages, blog posts or products
With OOP you can think it is an instant of a class.

In our example it is Event content item which is an actual item that we work with.

### Content Type

**Content type** is like a class of content item. Before creating a content type, we must define content type first.

Those three examples also describe three content types: page, blog post and product.
In other words what we call a blog post is just an item of type blog post.
For our example, we will create an Event content item from Event content type.

### Content Part

In Orchard, a content type is built from multiple smaller parts, are conveniently called content parts.
A content part is atom of content that is enough to build a specific coherent behavior and it can be reused across content types.
...
a numeric field representing its price,
and another numeric field representing its weight.
Each of these fields probably only makes sense on a product type.

### Define a new content type

In our example, we want to create a web page show an event information.
...
Autoroute, Title, HTML Body are existing parts in Orchard CMS.
They are in OrchardCore.Autoroute, OrchardCore.Title, OrchardCore.Html projects (modules) respectively.
However, we don't have a built-in Map part so we will create a new one.
...
Actually it is a .NET Core project.
We don't need to create it manually, we can use dotnet tool to create a module with scaffold content part for us.

-   Install .NET Core 3.1 from https://dotnet.microsoft.com/download/dotnet-core/3.1
-   Launch a new PowerShell and use the following command to install a new project template.
...
-   CD to a folder that you want to save OrchardCore source code and then clone a project's source code if you haven't had Orchard on your local machine.
...
-   Create a new module with a name that you want, in this example, we are going to create "Codesanook.Map" module for providing Map part which can be use in event content type
...
_Note_
**Part** suffix is appended automatically to the end of the supplied PartName.

### Update a module project

-   Open OrchardCore.sln which is in the root folder with Visual Studio.
-   Include the existing project Codesanook.Map under OrchardCore.Modules folder to the solution.
...
screenshot of the module project after including in the solution under OrchardCore.Modules folder
...
Set up a new website have you haven't setup it, use blog recipe and local SQL Server as database.
Go to admin /admin
...
Go to Model folder (Codesanook.Map/Modules), you will find we have generated MapPart.cs file.
This file is our main model. It is a content part that we will add feature.
Add new properties to the model and it will look like .
...
All method has pair of async method that we can override.
...
### What is Content Shape ??

It is a small object that contains all the information required in order to display on the screen.
Before a content item. Everything get mapped into a tree of shapes that is a sort of abstract representation of the contents of the final page.
The advantage of such trees of shapes is that any module can modify existing shapes or create new ones.

In MapPartDisplayDriver.cs in Display method. We create a IDisplayResult by using Combine method.
This combine two shapes which is MapPart for showing map in details page and MapPart_Summary for showing map part in summary (list page)
We return two shape and Orchard know how to render it correctly depend on display type Details or Summary

Let's dig into how we create a MapPart_Summary shape
...
We use Initialize with a generic of view model "MapPartViewModel" that will be passed to a view of shape type "MapPart"
and we construct a Lambda to create a view model with BuildViewModel method.

Orchard uses convention to map a view (\*.cshtml or \*.liquid) and shape name.
Given our shape type MapPart, it will be use MapPart.cshtml or MapPart.liquid in Views folder.
...
-   As you can see, we expand set the part properties, reference to origin part object and reference to a content item that this part belong to.

-   Go to BuildViewModel
...
-   This method create a MapPart_Edit shape that will use MapPart_Edit.cshml for show edit page of Map content part.
...
This method use updater.TryUpdateModelAsync to update our Map part and save to a database and take HTTP form data as input.
We explicit update only Latitude and Longitude.
It will return shape the result of Edit method which is MapPart_Edit shape.
Therefore, this method will end up with returing and edit page with a new page from a database.

Update a Startup file to
...
We use contentDefinitionManager.AlterPartDefinition to create a part with a part name which match with our part model which is a underlying model to store a part data.
Since Orchard core store data in MapPart in JSON document format, so we don't need to define any table/columns like Orchard version 1.
Therefore, our migration much simpler.
...
-   Remove Handlers folder and files inside, we don't need theme for our project. Learn Handler more.
-   Remove MapPartDisplayDriver.GetMapPartSettings method
-   Remove Settings folder that we don't use setting in this example
-   Remove setting view MapPartSettings.Edit
...
### Enable a module that contains Map part

Now you have a MapPart is ready to use.
In our example it will be Event content type.
We can attach a content part to a content type via code or UI in the admin.
To simplify our example, we will start with using admin panel.
Before using Map part, we need to enable Codesanook.Map module.

-   Go to admin menu on left-hand side click Configuration > Features > Search "Map" > Click enable
-   Verify if you have Map part ready to use.
-   Go Content menu > Content Definition > Content Parts > Search "Map"
-   Verify if you have Map content part

screenshot of steps to enable a module

![step to enable a module](./assets/images/steps-to-enable-module.png)

### Create an Event content type and attach Map part
...
-   You can sort the order of part by drag and drop
...
-   Click Publish
-   You will be redirect to content item list page, click view button to view an event you just created on frontend.
